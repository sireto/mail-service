...
const ActionsColumn = ({ row }: ActionsColumnProps) => {
    ...
    return (
        <div className='flex space-x-4 text-primary items-center'>
            <Modal
                triggerButton={
                    <ScanEye size={20} className='transition-all duration-300 ease-in-out hover:scale-105 cursor-pointer'/>
                }
                dialogBody={<PreviewFrame mjml={ mjml } modalTitle={row.original.name} modalDescription={"Preview your template"}/>}
