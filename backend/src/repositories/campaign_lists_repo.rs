...
pub trait CampaignListRepository {
    async fn add_lists_to_campaign(&self, campaign_id: Uuid, list_id: Vec<Uuid>) -> Result<Vec<NewListInCampaign>, diesel::result::Error>;
    async fn delete_lists_from_campaign(&self, campaign_id: Uuid, list_ids: Vec<Uuid>) -> Result<usize, diesel::result::Error>; 
...
impl CampaignListRepository for CampaignListRepositoryImpl {
    async fn add_lists_to_campaign(&self, campaign_id: Uuid, list_ids: Vec<Uuid>) -> Result<Vec<NewListInCampaign>, diesel::result::Error> {
        let mut conn = get_connection_pool().await;

        let new_lists: Vec<NewListInCampaign> = list_ids.into_iter().map(|list_id| NewListInCampaign { campaign_id, list_id }).collect();
        ...
        diesel::insert_into(campaign_lists::table)
            ...
            .map(|results| {
                results.into_iter().map(|(campaign_id, list_id)| NewListInCampaign { campaign_id, list_id }).collect()
            })
    }

    async fn delete_lists_from_campaign(&self, c_id: Uuid, list_ids: Vec<Uuid>) -> Result<usize, diesel::result::Error> {
    ...
    async fn get_lists_from_campaign(&self, c_id: Uuid) -> Result<Vec<ListResponse>, diesel::result::Error> {
        ...
        let result = campaign_lists
            ...
            .into_iter()
            .map(|(l_id, l_name, l_namespace_id, l_description, l_created_at, l_updated_at)| {
                ListResponse {
                    id: l_id,
