...
pub async fn delete_campaign(Path(campaign_id): Path<String>) -> Result<Json<DeleteCampaignResponse>, AppError> {
    let uuid_id = Uuid::parse_str(&campaign_id)?;
...
pub async fn send_campaign_email(Path(campaign_id): Path<String>) -> Result<Json<AddMailToQueueResponse>, AppError> {
    let uuid_id = Uuid::parse_str(&campaign_id)?;
...
pub async fn send_campaign_email_smtp(Path(campaign_id): Path<String>) -> Result<Json<CampaignSendResponse>, AppError> {
    ...
    let result = send_campaign_email_smtp(campaign_uuid, server_uuid)
        .await
