...
use crate::{
    ...
    models::{
        campaign::{
            AddMailToQueueResponse, CampaignSendResponse, DeleteCampaignResponse, GetCampaignResponse,
            UpdateCampaignRequest, UpdateCampaignResponse,
        },
        campaign_lists::NewListInCampaign,
...
impl CampaignListService {
    ...
    pub async fn add_lists_to_campaign(
        &self,
        campaign_id: Uuid,
        list_ids: Vec<Uuid>,
...
pub async fn update_campaign(
    campaign_id: Uuid,
    payload: UpdateCampaignRequest,
) -> Result<UpdateCampaignResponse, AppError> {
    let campaign_repository = Arc::new(CampaginRepositoryImpl);
    let campaign_service = CampaignService::new(campaign_repository);
    ...
    let existing_lists = campaign_list_service
        .get_lists_from_campaign(updated_campaign_response.id)
        .await
        .map_err(|err| AppError::NotFoundError(Some(err.to_string())))?;
    ...
    let to_add: Vec<Uuid> = new_list_ids.difference(&existing_list_ids).cloned().collect();
...
pub async fn send_campaign_email(campaign_id: Uuid) -> Result<AddMailToQueueResponse, AppError> {
...
pub async fn send_campaign_email_aws(campaign_id: Uuid) -> Result<CampaignSendResponse, AppError> {
    ...
    let sender_id_string = match campaign.campaign_senders {
        ...
        None => {
            return Err(AppError::NotFoundError(Some(
                "Sender ID not found for campaign.".to_string(),
            )))
        } // Handle None case
    };

    let campaign_sender_response = get_campaign_sender_by_id(sender_id_string)
        .await
        .map_err(|err| AppError::NotFoundError(Some(err.to_string())))?;
    ...
    for contact in contacts.clone() {
        ...
        let result = client
            ...
            .destination(Destination::builder().to_addresses(contact.email.clone()).build())
...
pub async fn add_lists_to_campaign(campaign_id: Uuid, list_ids: Vec<Uuid>) -> Result<Vec<NewListInCampaign>, AppError> {
    ...
    let result = campaign_list_service
        .add_lists_to_campaign(campaign_id, list_ids)
        .await?;
...
pub async fn send_single_email(
    ...
    match server_type {
        ...
        ServerTypeEnum::SMTP => {
            ...
            server_service
                .send_mail_with_smtp(server_id, &from_email, vec![email], None, None, &subject, &message)
                .await
                .map_err(|err| AppError::InternalServerError(Some(format!("{:?}", err))))?;
        }
    };
    ...
    mail_service
        ...
        .await
        .map_err(|err| AppError::InternalServerError(Some(format!("Failed to update mail status: {}", err))))?;
