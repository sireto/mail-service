...
pub async fn add_bounce(payload: CreateBounceLogRequest) -> Result<CreateBounceLogResponse, AppError> {
...
pub async fn get_all_bounces() -> Result<Vec<BounceLog>, AppError> {
    let bounce_logs_repository = Arc::new(BounceLogsRepositoryImpl);
    let bounce_logs_service = BounceLogsService::new(bounce_logs_repository);
