...
use crate::{
    error::AppError,
    models::{
        contact::{Contact, ContactList, GetContactResponsee},
        list::{
            CreateListRequest, CreateListResponse, DeleteListResponse, ListResponse, UpdateListRequest,
            UpdatedListResponse,
        },
        list_contacts::NewContactInList,
    },
    repositories::{
...
impl ListService {
    ...
    pub async fn create_list(&self, payload: CreateListRequest) -> Result<CreateListResponse, AppError> {
    ...
    pub async fn get_all_lists(&self, namespace_id: Uuid) -> Result<Vec<ListResponse>, AppError> {
        let all_lists = self.repository.get_all_lists(namespace_id).await.map(|lists| {
            lists
                .into_iter()
    ...
    pub async fn get_list_by_id(&self, namespace_id: Uuid, list_id: Uuid) -> Result<ListResponse, AppError> {
        let list = self
            .repository
    ...
    pub async fn delete_list(&self, namespace_id: Uuid, list_id: Uuid) -> Result<DeleteListResponse, AppError> {
...
pub async fn delete_list(namespace_id: Uuid, list_id: Uuid) -> Result<DeleteListResponse, AppError> {
    let list_repository = Arc::new(ListRepositoryImpl);
    let list_service = ListService::new(list_repository);

    let deleted_list = list_service
        .delete_list(namespace_id, list_id)
        .await
        .map_err(|err| AppError::NotFoundError(Some(err.to_string())))?;
...
pub async fn add_contacts_to_list(list_id: Uuid, contact_ids: Vec<Uuid>) -> Result<Vec<NewContactInList>, AppError> {
    ...
    let new_contact_in_list = contact_service.add_contacts_to_list(list_id, contact_ids).await?;

    Ok(new_contact_in_list)
}

pub async fn delete_contacts_from_list(list_id: Uuid, contact_ids: Vec<Uuid>) -> Result<usize, AppError> {
    let list_contact_repository = Arc::new(ListContactRepositoryImpl);
    let contact_service = ListContactService::new(list_contact_repository);

    let deleted_contact_from_list = contact_service.delete_contacts_from_list(list_id, contact_ids).await?;

    Ok(deleted_contact_from_list)
}

pub async fn get_contacts_from_lists(list_ids: Vec<Uuid>) -> Result<Vec<GetContactResponsee>, (StatusCode, String)> {
    let list_contact_repository = Arc::new(ListContactRepositoryImpl);
    let contact_service = ListContactService::new(list_contact_repository);
    ...
    let contacts = contact_service
        .get_contacts_from_lists(list_ids.clone())
        .await
    ...
    let list_contacts = contact_repository
        .get_list_contacts(contact_ids)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    // Get all lists these contacts belong to
    let all_list_ids: Vec<Uuid> = list_contacts.iter().map(|lc| lc.list_id).collect();
    let lists = contact_repository
        .get_lists_by_ids(all_list_ids)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
