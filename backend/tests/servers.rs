...
async fn test_get_all_servers() {
    let mut mock_repo = MockServerRepo::new();
    let expected_servers = vec![Server {
        id: Uuid::new_v4(),
        active: true,
        host: "smtp.example.com".to_string(),
        smtp_username: "user@example.com".to_string(),
    ...
    mock_repo
        ...
        .returning(move || Ok(expected_servers.clone()));
    let server_service = ServerService::new(Arc::new(mock_repo));
...
async fn test_update_server() {
    ...
    mock_repo
        ...
        .returning(move |_, _| Ok(updated_server.clone()));
    let server_service = ServerService::new(Arc::new(mock_repo));
    let result = server_service
        .update_server(&server_id.to_string(), update_payload)
        .await;
    assert!(result.is_ok());
